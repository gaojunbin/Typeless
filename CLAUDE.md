...
Language rule: the product UI is bilingual (Chinese by default, English selectable) and every piece of interface copy lives in `src/renderer/i18n/messages/*.ts` in both languages; all source, comments, docs and commit messages are English.
...
npm run test:update-mac    # macOS only: installs a fake newer build over a packaged copy in .local/ and checks the relaunch (package:mac first)
...
npm run package:mac        # electron-builder arm64 DMG -> release/, sealed with the fixed 'Typeless Project' certificate (TYPELESS_SIGN_IDENTITY=- for ad hoc)
...
- Renderer layout: `main.tsx` renders the first-run setup guide (`onboarding/SetupGuide.tsx`, spec in `docs/UI_DESIGN.md` section 12) while `settings.general.setupCompleted` is false; afterwards it owns the current `Page` (three destinations: 首页, AI 配置, 基本设置) and renders `Sidebar.tsx` plus one page — `Home.tsx` (`DictationPanel.tsx` plus a three-row status list), `settings/Providers.tsx` (AI 配置: both provider forms; the 文字润色 form embeds the 润色程度 and 个人表达说明 rows from `settings/Writing.tsx`), `settings/Preferences.tsx` (基本设置 including the 关于 row with the release check). Colour is reserved for status badges and the microphone meter; pages are white. Interface copy: `src/renderer/i18n/index.ts` exports `useI18n()` (`{ language, t }`) for components and `translate(language, key, params)` for plain modules; the text lives in `i18n/messages/{common,home,session,settings,onboarding}.ts`, each declared with `defineMessages` so the Chinese and English objects must share exactly the same keys (`tests/i18n.test.ts` also checks placeholders and that English has no CJK). Add every new string to the catalogue in both languages; never hard-code copy in a component. The e2e suites select by the Chinese copy, so keep it byte-identical when refactoring. Shared primitives (`PageHeader`, `SectionGroup`, `SettingRow`, `Segmented`, `KeyChips`, `StatusBadge`, `Wave`) live in `ui.tsx`; design tokens and shared CSS in `styles/tokens.css` and `styles/base.css`, page CSS next to each page (`styles/*.css`, imported by the component). `docs/UI_DESIGN.md` is the visual contract: keep the accessibility hooks it lists in section 10, because the e2e suites select by them.
...
- Release check: `UpdateChecker` fetches `TYPELESS_UPDATE_URL` (default: this repo's GitHub `releases/latest`) 15 s after launch and every 6 h when packaged or when the variable is set (`off` disables the automatic check; the manual 检查更新 button always runs). It downloads the platform asset into `<data root>/downloads` when `TYPELESS_DATA_DIR` is set, else the system Downloads folder, verifies the GitHub `digest` when present, and on macOS installs in place (`electron/update-installer.ts`: `hdiutil attach` → bundle id / version / `codesign --verify` checks → `ditto` beside the installed bundle → two renames → detach) before relaunching through `open`; a failed install keeps the file for `shell.openPath`. The installer also refuses a bundle whose designated requirement differs from the installed copy's unless the installed copy is ad hoc (the pre-2.4.0 state), so a certificate rotation means one manual install. Windows only opens the archive. Pure logic lives in `src/core/update.ts`; state travels as `AppSnapshot.update`. `npm run test:update-mac` exercises the real swap with a packaged copy under `.local/`.
...
- Error codes are strings like `asr_401`, `no_speech`, `clipboard_changed`. The renderer maps them to copy in `src/renderer/sessionPresentation.ts` through the `session.recovery.*` / `session.warning.*` catalogue keys; new codes need an entry there and in `recoverySettings` if they should link to a settings tab.
...
- `general.language` (`'zh' | 'en'`, default `'zh'`) selects the interface language; the store rejects other values, `main.tsx` wraps the tree in `I18nProvider` and sets `document.documentElement.lang`, and the main process rebuilds the tray menu from the same setting. The switch lives in 基本设置 → 通用 and on the setup guide's welcome screen.
...
- `docs/PROPOSAL.md` is the current product/design contract and `docs/UI_DESIGN.md` the interface specification; `docs/VALIDATION.md` records what was actually executed for the current version and its limits; `README.md` is the short user-facing entry and `docs/DEVELOPMENT.md` the build/run/test/package entry; `docs/USER_GUIDE.md` describes user-facing behavior; `docs/research/` is historical evidence, not a feature list. `RESEARCH_BRIEF.md` and `SYNTHESIS_CHECKLIST.md` at the root are earlier planning artifacts.
...
- GitHub release notes are English only and short: a one-line summary, the list of what changed, the download table with SHA-256 digests, and nothing else. No upgrade essays or repeated background.
