...
Date: 2026-09-18, revised 2026-09-23. Target version: 2.4.0. This document records the reduced product decision. Actual package availability and completed acceptance belong in [Validation](VALIDATION.md), not in this proposal.
...
Version 2.4.0 packaging targets macOS Apple silicon DMG and Windows x64 portable ZIP. Package builds do not imply signing, notarization, publication or platform-runtime acceptance. The application checks GitHub Releases for a newer version and downloads its installer on request, verifying the published digest. On macOS it then installs in place (mounts the image, verifies the bundle, requires the same signing certificate as the installed copy, swaps it into the Applications folder, unmounts) and relaunches. Releases are sealed with one fixed self-signed project certificate rather than ad hoc, so the system permission grants survive upgrades; the certificate stays in the maintainer's keychain and only its name is in the repository. Gatekeeper still warns on first install because no Apple certificate is involved. On Windows it opens the archive for a manual install. The [development guide](DEVELOPMENT.md) supplies commands and output paths; [Validation](VALIDATION.md) records the evidence.
