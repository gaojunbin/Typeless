...
describe('dictation recovery presentation', () => {
  it('keeps a numeric review warning alongside uncertain delivery instead of claiming paste failed', () => {
    const message = warningMessage({ ...idleSession(), copied: true, delivery: 'copied', errorCode: 'native_timeout', warning: 'Numbers changed during text processing. Review before insertion.' });
  ...
  it.each(['paste_uncertain', 'insertion_uncertain', 'native_timeout', 'native_protocol_error', 'helper_unavailable', 'native_error'])('does not encourage duplicate paste after %s', errorCode => {
    const message = warningMessage({ ...idleSession(), copied: true, delivery: 'copied', errorCode });
  ...
  it('identifies the earlier automatic paste without implying that copying raw text pastes it again', () => {
    ...
    expect(warningMessage(session)).toBe('整理后的文字已尝试自动粘贴，请检查当前应用。');
    expect(warningMessage({ ...session, rawText: session.text })).toBe('本次听写已尝试自动粘贴，请检查当前应用。');
  });

  it('does not claim the clipboard still holds output after another app changed it', () => {
    const message = warningMessage({ ...idleSession(), copied: true, errorCode: 'clipboard_changed' });
  ...
  it('keeps polishing failure visible even when copying succeeded', () => {
    ...
    expect(recoveryMessage(session)).toContain('润色未完成');
    expect(recoveryMessage(session)).toContain('识别原文');
  });

  it('separates rate limiting from payment errors and routes configuration failures', () => {
    expect(recoveryMessage({ ...idleSession(), errorCode: 'asr_429' })).toContain('请求过多');
    expect(recoveryMessage({ ...idleSession(), errorCode: 'asr_429' })).not.toMatch(/余额|额度/);
    expect(recoveryMessage({ ...idleSession(), errorCode: 'asr_402' })).toContain('余额不足');
  ...
  it('offers nearby microphone settings without exposing internal error text or a circular instruction', () => {
    ...
    expect(recoveryMessage(session)).toContain('未检测到声音');
    expect(recoveryMessage(session)).not.toMatch(/Synthetic|打开 Typeless/);
