...
describe('configuration application', () => {
  it('does not reconfigure shortcuts for unrelated saves and distinguishes persisted settings from OS apply failure', async () => {
    ...
    const controller = new Controller(store, provider, { ...host, settingsChanged, publish: vi.fn(), permissions: vi.fn(), openPane: vi.fn(), microphoneTest: vi.fn(), show: vi.fn(), hide: vi.fn(), quit: vi.fn(), relaunch: vi.fn(), copyDiagnostics: vi.fn(async () => {}), checkUpdate: vi.fn(async () => {}), downloadUpdate: vi.fn(async () => {}), openUpdate: vi.fn(async () => {}), installUpdate: vi.fn(async () => {}), openReleasePage: vi.fn(async () => {}) }, 'test');
