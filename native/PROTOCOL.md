...
Events are `shortcut:{action:"toggle"}` and `ready:{platform}`. Isolated Fn/Right Alt release toggles recording. Chords suppress the candidate, never the original OS event. No focus polling or target-change events remain. OS shortcut conflicts are not changed automatically.
...
A tap that keeps failing while the process is authorized cannot be repaired in place. The helper reports the failing reason and keeps reconciling; it never exits on its own, and no environment variable or exit code asks the client to restart it. Field testing showed that a fresh helper process does not make the operating system hand out a working tap. The shell therefore presents `tap_creation_failed`, `tap_disabled`, or `runloop_source_failed` on an authorized helper as `relaunch_required` and offers a full application relaunch; a grant recorded for an earlier installation identity has to be removed and added again in System Settings.
